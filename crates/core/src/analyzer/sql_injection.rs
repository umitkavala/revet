...
mod tests {
    ...
    fn test_should_scan_source_files() {
        assert!(SqlInjectionAnalyzer::should_scan(&PathBuf::from("app.py")));
        assert!(SqlInjectionAnalyzer::should_scan(&PathBuf::from(
            "routes.ts"
        )));
        assert!(SqlInjectionAnalyzer::should_scan(&PathBuf::from(
            "handler.js"
    ...
    fn test_is_comment_line() {
        assert!(SqlInjectionAnalyzer::is_comment_line(
    ...
    #[test]
    fn test_orm_raw_pattern() {
        let pats = patterns();
        let pat = &pats[0]; // ORM raw query (most specific, checked first)
        assert!(pat
            .regex
            .is_match(r#"User.objects.raw(f"SELECT * FROM users WHERE name = '{name}'")"#));
    }

    #[test]
    fn test_fstring_execute_pattern() {
        let pats = patterns();
        let pat = &pats[1];
        assert!(pat
            .regex
    ...
    #[test]
    fn test_concat_execute_pattern() {
        let pats = patterns();
        let pat = &pats[2];
        assert!(pat
            .regex
    ...
    #[test]
    fn test_format_execute_pattern() {
        let pats = patterns();
        let pat = &pats[3];
        assert!(pat
            .regex
            .is_match(r#"cursor.execute("SELECT * FROM users WHERE id = {}".format(uid))"#));
    }

    #[test]
    fn test_percent_execute_pattern() {
        let pats = patterns();
        let pat = &pats[4];
        assert!(pat
            .regex
            .is_match(r#"cursor.execute("SELECT * FROM users WHERE id = %s" % uid)"#));
        // Parameterized query should NOT match — comma after closing quote
        assert!(!pat
            .regex
            .is_match(r#"cursor.execute("SELECT * FROM users WHERE id = %s", (uid,))"#));
    }
    ...
    #[test]
    fn test_template_literal_query_pattern() {
        let pats = patterns();
        let pat = &pats[5];
        assert!(pat
            .regex
            .is_match(r#"db.query(`SELECT * FROM users WHERE id = ${uid}`)"#));
    }
}
